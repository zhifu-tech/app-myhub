...
`datastore` 是 MyHub 应用的数据层模块套件，负责所有数据相关的操作，包括本地存储、网络请求、数据模型和业务逻辑。该套件采用模块化设计，将不同职责拆分为独立的子模块，以提高代码的可维护性和可测试性。

> **注意**：`datastore` 目录现在主要包含文档，实际的代码已拆分为多个子模块。
...
- 依赖 `datastore:model`（使用领域模型类型）
- 被数据库客户端/服务端模块依赖

---

#### 3. `datastore:database-client`
...
- 依赖 `datastore:database`（使用生成的数据库接口）
- 被 `datastore:datasource-local` 和 `datastore:repository-client` 依赖

---

#### 4. `datastore:database-server`
...
- 依赖 `datastore:database`
- 被 `datastore:repository-server` 依赖

---

#### 5. `datastore:database-test`
...
- 依赖 `datastore:database`（使用 `MyHubDatabase`）
- 依赖 `datastore:model`（使用领域模型）
- 被 `datastore:repository-client` 和 `datastore:repository-server` 依赖

---

#### 7. `datastore:datasource-remote`
...
- 依赖 `datastore:model`（使用领域模型和 DTO）
...
- **依赖注入**：
    - `RepositoryModule` - Koin 模块（客户端版本）
        - 包含 `databaseModule`（来自 `datastore:database-client`）
        - 包含 `localDataSourceModule`（来自 `datastore:datasource-local`）
        - 包含 `remoteDataSourceModule`（来自 `datastore:datasource-remote`）
        - 提供所有 Repository 实现（同时绑定基础接口和响应式接口）

**依赖关系**：

- 依赖 `datastore:repository`（接口）
- 依赖 `datastore:datasource-local`
- 依赖 `datastore:datasource-remote`
- 依赖 `datastore:database-client`
- 被 `composeApp` 依赖

---

#### 10. `datastore:repository-server`
...
```
repositoryModule (datastore:repository-client)
    ├── databaseModule (datastore:database-client)
    │       └── databaseDriverFactoryModule()
    ├── localDataSourceModule (datastore:datasource-local)
    │       └── 依赖 databaseModule 提供的 MyHubDatabase
    └── remoteDataSourceModule (datastore:datasource-remote)
            └── networkModule
                    ├── KtorClientFactory
                    └── HttpClient
```
...
```
repositoryModule (datastore:repository-server)
    ├── databaseModule (datastore:database-server)
    │       └── databaseDriverFactoryModule()
    └── localDataSourceModule (datastore:datasource-local)
            └── 依赖 databaseModule 提供的 MyHubDatabase
```
...
- **`datastore:model`**：纯数据模型，无业务逻辑，无依赖
- **`datastore:database`**：数据库 Schema 定义，生成类型安全的数据库接口
- **`datastore:database-client/server`**：平台特定的数据库驱动配置
- **`datastore:datasource-local`**：本地数据访问层，封装 SQLDelight 操作
- **`datastore:datasource-remote`**：远程数据访问层，封装 Ktor Client 操作
- **`datastore:repository`**：业务接口定义，定义数据访问契约
- **`datastore:repository-client/server`**：业务逻辑层，协调数据源，实现业务规则

### 3. 接口与实现分离

- **Repository 接口**（`datastore:repository`）与实现（`datastore:repository-client/server`）分离
...
- **`datastore:datasource-local`** - 本地数据源单元测试
...
- **`datastore:datasource-remote`** - 远程数据源单元测试
...
- **`datastore:repository-client`** - Repository 单元测试
...
- **`datastore:database-test`** - 数据库基础功能测试
    - `DatabaseTest` - Schema 创建、插入查询、事务回滚、外键约束等

### 测试工具

- `runDatabaseTest` - 跨平台数据库测试辅助函数（在 `datastore:database-test` 中）
- `createMockHttpClient` - Mock HTTP 客户端工具（在 `core:network-test` 中）
...
val appModule = module {
    includes(repositoryModule) // 来自 datastore:repository-client
}
...
val serverModule = module {
    includes(repositoryModule) // 来自 datastore:repository-server
}
