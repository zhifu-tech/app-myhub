...
```bash
# 运行所有平台的测试
./gradlew :datastore:model:allTests

# 运行特定平台的测试
./gradlew :datastore:model:jvmTest
./gradlew :datastore:model:jsTest
./gradlew :datastore:model:wasmJsTest
```
...
- `datastore:datasource-remote` - 远程数据源
- `datastore:repository` - 仓库接口
