...
- `core:datastore-database` - SQLDelight 数据库定义
...
模块包含完整的单元测试，使用 `core:datastore-database-test` 提供的测试工具。

### 运行测试

```bash
# 运行所有平台的测试
./gradlew :core:datastore-datasource-local:allTests

# 运行特定平台的测试
./gradlew :core:datastore-datasource-local:jvmTest
./gradlew :core:datastore-datasource-local:jsTest
./gradlew :core:datastore-datasource-local:iosSimulatorArm64Test
```
...
- `core:datastore-database-client` - 数据库驱动工厂
