...
**注意**：初始数据加载需要 `DatabaseManager` 在 Koin 中可用。确保 `core:datastore-database-manage` 模块已正确配置。
...
- `core:datastore-database` - 数据库 Schema 定义（提供 `MyHubDatabase`）
- `core:datastore-database-manage` - 数据库管理工具（用于初始数据加载）
...
1. **SQLite 初始数据加载**：
   ...
   - 确保 `core:datastore-database-manage` 模块已正确配置
...
- `core:datastore-database-client` - 客户端数据库驱动（对比参考）
- `core:datastore-database-manage` - 数据库管理工具（初始数据加载）
- `core:datastore-repository-server` - 服务端数据仓库（使用此模块）
...
- 检查 `core:datastore-database-manage` 模块是否正确配置
...
| 特性 | `core:datastore-database-client` | `core:datastore-database-server` |
|------|----------------------------------|-----------------------------------|
