...
**重要说明**：`core/datastore` 是一个**混合模块套件**，包含多个子模块，每个子模块专注于特定的职责。整体采用分层架构设计，由上而下分为：**Repository 层**、**Datasource 层**、**Database 层**。
...
- **`core:datastore-repository-client`**：客户端 Repository 实现
  - 协调 `LocalDataSource` 和 `RemoteDataSource`
  - 实现响应式接口（`observe*()` 方法）
  - 实现数据同步逻辑

- **`core:datastore-repository-server`**：服务端 Repository 实现
  - 仅使用 `LocalDataSource`
  - 封装业务逻辑
  - 确保用户隔离
...
- **`core:datastore-datasource-remote`**：远程数据源实现
...
- **`core:datastore-database-client`**：客户端数据库配置
  - `DatabaseDriverFactory`（expect/actual）
  - 平台特定的驱动实现（Android、iOS、JVM、JS）
  - Koin DI 模块

- **`core:datastore-database-server`**：服务端数据库配置
  - 服务端特定的驱动实现（JVM）
  - 支持 SQLite 和 PostgreSQL（通过环境变量配置）
  - Koin DI 模块

- **`core:datastore-database-test`**：数据库测试工具
  - `runDatabaseTest` 辅助函数
  - 跨平台测试支持
  - 内存数据库创建

- **`core:datastore-database-manage`**：数据库管理工具
  - 初始数据加载（从 JSON 资源文件）
  - 数据清理功能
  - 用户数据关联
...
val repositoryModule = module {
    // Database
    includes(databaseModule) // 来自 core:datastore-database-client
    
    // DataSource
    includes(localDataSourceModule) // 来自 core:datastore-datasource-local
    includes(remoteDataSourceModule) // 来自 core:datastore-datasource-remote
...
val repositoryModule = module {
    // Database
    includes(databaseModule) // 来自 core:datastore-database-server
...
- ✅ **阶段 1**：数据模型层设计（`core:datastore-model`）
- ✅ **阶段 2**：数据库层设计（`core:datastore-database`、`core:datastore-database-client`、`core:datastore-database-server`）
- ✅ **阶段 3**：数据源层设计（`core:datastore-datasource-local`、`core:datastore-datasource-remote`）
- ✅ **阶段 4**：仓库层设计（`core:datastore-repository-client`、`core:datastore-repository-server`）
- ✅ **阶段 5**：总体架构设计（本文档）
...
- **产出**：
  - `core:datastore-database` 模块
  - `core:datastore-database-client` 模块
  - `core:datastore-database-server` 模块
  - `core:datastore-database-test` 模块
  - `core:datastore-database-manage` 模块
- **时间**：已完成
...
- **目标**：实现本地和远程数据源
- **产出**：
  - `core:datastore-datasource-local` 模块
  - `core:datastore-datasource-remote` 模块
- **时间**：已完成
...
- **目标**：实现客户端和服务端仓库
- **产出**：
  - `core:datastore-repository` 模块（接口）
  - `core:datastore-repository-client` 模块
  - `core:datastore-repository-server` 模块
- **时间**：已完成
