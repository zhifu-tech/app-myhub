    ...
    override suspend fun updateCard(card: Card) {
        database.transaction {
            ...
            if (card.metadata != null) {
                database.cardQueries.insertCardMetadata(
                    card_id = card.id,
                    quote_author = card.metadata.quoteAuthor,
                    quote_category = card.metadata.quoteCategory,
                    code_language = card.metadata.codeLanguage,
                    code_snippet = card.metadata.codeSnippet,
                    article_url = card.metadata.articleUrl,
                    article_summary = card.metadata.articleSummary,
                    article_image_url = card.metadata.articleImageUrl,
                    word_pronunciation = card.metadata.wordPronunciation,
                    word_definition = card.metadata.wordDefinition,
                    word_example = card.metadata.wordExample,
                    idea_priority = card.metadata.ideaPriority,
                    idea_status = card.metadata.ideaStatus
                )

                // 更新待办清单项（先删除再插入）
                database.cardQueries.deleteChecklistItems(card.id)
                card.metadata.checklistItems.forEach { item ->
                    database.cardQueries.insertChecklistItem(
                        id = item.id,
                        card_id = card.id,
                        text = item.text,
