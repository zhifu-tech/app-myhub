...
1. [问题背景](#1-问题背景)
2. [Now in Android 架构分析](#2-now-in-android-架构分析)
3. [KMP 导航技术方案对比](#3-kmp-导航技术方案对比)
4. [MyHub 导航方案设计](#4-myhub-导航方案设计)
5. [核心概念详解](#5-核心概念详解)
6. [实现细节](#6-实现细节)
7. [实施计划](#7-实施计划)
8. [风险评估](#8-风险评估)
...
- [官方导航文档技术点分析](./myhub-navigation-official-docs-analysis.md) - 详细技术调研
- [KMP Navigation 能力全景图](./myhub-navigation-capability-map.md) - 能力对比可视化
- [KMP 导航库调研与评估](./myhub-navigation-library-evaluation.md) - 库详细对比
...
| 版本     | 日期         | 修改内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | 修改原因                 |
|--------|------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|
| v1.0.1 | 2026-01-11 | 修正实施过程中发现的技术问题：<br>**1. NavKey 序列化支持**<br> - 添加 `@Serializable` 注解到所有 NavKey 实现<br> - 确保 `rememberNavBackStack` 能够保存和恢复状态<br>**2. SavedStateConfiguration 配置**<br> - 在 `rememberAppNavigationState` 中显式配置 `SavedStateConfiguration.DEFAULT`<br> - 支持跨配置变更的状态持久化<br>**3. NavBackStack 构造修正**<br> - 修正构造函数调用：`NavBackStack(key)` 而非 `NavBackStack(initialKey = key)`<br> - 使用 vararg elements 参数而非具名参数<br>**4. ViewModelStore 装饰器移除**<br> - 移除 `rememberViewModelStoreNavEntryDecorator`（KMP 不支持）<br> - 从 `build.gradle.kts` 移除 `lifecycle-viewmodel-navigation3` 依赖<br> - 更新文档说明仅使用 `SaveableStateHolder` 装饰器<br>**5. 文档与代码同步**<br> - 所有示例代码已同步为实际可用的 KMP 代码<br> - 标注平台限制和注意事项 | Bugfix：代码实施与设计文档规范对齐 |
...
本版本（v1.0.1）是在实际代码落地过程中发现的技术问题修正。修正内容不影响架构设计，仅涉及 API 细节和 KMP 平台兼容性。

### 修正清单

#### 1. NavKey 序列化支持（必须）

**问题**：Navigation 3 的 `rememberNavBackStack` 需要序列化支持才能保存和恢复状态，但 v1.0 文档中未明确要求 NavKey 标记 `@Serializable`。
...
**影响**：所有 NavKey 实现必须标记 `@Serializable`，否则运行时会抛出序列化异常。

---

#### 2. SavedStateConfiguration 显式配置（建议）

**问题**：`rememberNavBackStack` 需要显式提供 `SavedStateConfiguration` 以支持状态持久化。
...
**影响**：如需自定义序列化器，可使用 `SavedStateConfiguration { ... }` 工厂函数。

---

#### 3. NavBackStack 构造函数修正（必须）

**问题**：v1.0 文档示例中使用 `NavBackStack(initialKey = key)`，但实际 API 是 `NavBackStack(vararg elements: E)`。
...
val subStacks = remember(appKeys) {
    appKeys.associateWith { key ->
        NavBackStack(initialKey = key)  // 错误：不存在 initialKey 参数
    }
}
...
val subStacks = remember(appKeys) {
    appKeys.associateWith { key ->
        NavBackStack(key)  // 正确：使用 vararg 参数
    }
}
```
...
#### 4. ViewModelStore 装饰器移除（KMP 限制）

**问题**：`lifecycle-viewmodel-navigation3` 依赖仅支持 Android 平台，在 KMP 项目中不可用。

**解决方案**：

1. **移除依赖**（`core/navigation/build.gradle.kts`）：

```kotlin
commonMain {
    dependencies {
        api(libs.navigation3.ui)
        // ❌ 移除：implementation("androidx.lifecycle:lifecycle-viewmodel-navigation3:2.10.0")
    }
}
```

2. **移除装饰器**（`AppNavigationState.kt`）：
...
fun AppNavigationState.toEntries(
    ...
    val decoratedEntries = subStacks.mapValues { (_, stack) ->
        val decorators = listOf(
            ...
            // ❌ 移除：rememberViewModelStoreNavEntryDecorator<NavKey>(),
        )
...
- ✅ Composable 状态管理正常（`rememberSaveable`、`remember` 等）
- ❌ ViewModel 需使用其他方案（Koin、StateFlow 等）
...
- 所有示例代码已更新为实际可运行的 KMP 代码
- 添加 `⚠️ v1.0.1 修正` 注释标记变更点
- 补充平台限制说明（如 ViewModelStore 仅 Android 支持）

---

### 修正影响评估

| 修正项                     | 影响范围                         | 兼容性         | 是否破坏性变更         |
|-------------------------|------------------------------|-------------|-----------------|
| NavKey 序列化              | 所有 NavKey 实现                 | 向前兼容        | ❌ 否（新增要求）       |
| SavedStateConfiguration | `rememberAppNavigationState` | 向前兼容        | ❌ 否（API 增强）     |
| NavBackStack 构造         | SubStack 创建逻辑                | 不兼容（API 修正） | ✅ 是（但 v1.0 未落地） |
| ViewModelStore 移除       | 状态管理                         | 功能降级        | ✅ 是（但可用替代方案）    |
...
// 应用级导航键
// ⚠️ v1.0.1 修正：所有 NavKey 必须标记 @Serializable，以便 rememberNavBackStack 能够保存和恢复状态
@Serializable
sealed interface AppNavKey : NavKey {
    @Serializable
    data object Dashboard : AppNavKey
...
fun rememberAppNavigationState(
    ...
    // ⚠️ v1.0.1 修正：添加 SavedStateConfiguration 支持状态持久化
    val savedStateConfiguration = remember {
        SavedStateConfiguration.DEFAULT
    }
    val appStack = rememberNavBackStack(savedStateConfiguration, startKey)
    ...
    // ⚠️ v1.0.1 修正：NavBackStack 构造函数只接受 vararg elements，不接受 initialKey 参数
    val subStacks = remember(appKeys) {
        appKeys.associateWith { key ->
            NavBackStack(key)
        }
...
fun AppNavigationState.toEntries(
    ...
    // ⚠️ v1.0.1 修正：移除 ViewModelStore 装饰器，因为它在 KMP 中不支持
    // 如需 ViewModel 功能，应使用其他状态管理方案（如 Koin、StateFlow 等）
    val decoratedEntries = subStacks.mapValues { (_, stack) ->
        val decorators = listOf(
            rememberSaveableStateHolderNavEntryDecorator<NavKey>(),
        )
...
- ⚠️ **v1.0.1 修正**：NavKey 必须标记 `@Serializable`，以便 `rememberNavBackStack` 能够保存和恢复状态（使用 SavedStateConfiguration）
...
2. **`rememberViewModelStoreNavEntryDecorator`** ⚠️ **KMP 不支持**
    - **作用**：为每个 NavEntry 提供独立的 `ViewModelStore`（仅 Android 平台）
    ...
    - **⚠️ v1.0.1 修正**：在 KMP 项目中已移除，因为 `lifecycle-viewmodel-navigation3` 仅支持 Android 平台。如需 ViewModel 功能，应使用其他状态管理方案（如 Koin、StateFlow 等）
...
- ✅ **状态隔离**：每个 NavEntry 有独立的状态管理作用域，互不干扰
...
// ⚠️ v1.0.1 修正：ViewModelStore 在 KMP 中不支持
// 如需 ViewModel 功能，应使用其他状态管理方案（如 Koin）
@Composable
fun CardDetailScreen(cardId: String) {
    // 使用 Koin 或其他 KMP 兼容的状态管理方案
    val viewModel: CardDetailViewModel = koinInject<CardDetailViewModel>()
    // ViewModel 的生命周期需要手动管理，或使用其他状态管理方案
}
```
...
1. [官方导航文档技术点分析](./myhub-navigation-official-docs-analysis.md)
2. [KMP Navigation 能力全景图](./myhub-navigation-capability-map.md)
3. [KMP 导航库调研与评估](./myhub-navigation-library-evaluation.md)
...
- ✅ **状态管理**：状态装饰器模式（SaveableStateHolder，⚠️ ViewModelStore 仅 Android 支持，已从 KMP 版本移除）
