...
```text
core/datastore-database-test/src/
├── commonMain/
...
```bash
# 运行所有平台的测试
./gradlew :core:datastore-database-test:allTests

# 运行特定平台的测试
./gradlew :core:datastore-database-test:jvmTest
./gradlew :core:datastore-database-test:jsTest
# 注意：wasmJsTest 已被禁用（由于 JS interop 限制）
# 数据库测试在 WASM 平台会自动跳过，推荐使用 jsTest 进行 Web 数据库测试
./gradlew :core:datastore-database-test:iosSimulatorArm64Test
```
...
- `core:datastore-database-client` - 客户端数据库驱动
- `core:datastore-datasource-local` - 本地数据源（使用此模块进行测试）
