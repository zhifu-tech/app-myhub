...
- `server/docs/architecture.md` 中的「推荐结构」已大部分落地（api / service 在 server 内，repository 在 datastore）。
...
3. **文档与实现对齐**
   - `architecture.md` 中 Statistics、Templates 命名与推荐结构中的 repository 位置，建议更新为与当前实现一致（含 repository 在 datastore、CardTemplates 命名等）。
