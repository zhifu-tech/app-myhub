package tech.zhifu.app.myhub.api.collection
...
fun Route.collectionsApi() {
    authenticate("auth-bearer") {
        route("/api/collections") {
            get {
                val collectionService = call.application.get<CollectionService>()
                try {
                    val userId = call.getCurrentUserId()
                    val page = call.request.queryParameters["page"]?.toIntOrNull() ?: 1
                    val pageSize = call.request.queryParameters["pageSize"]?.toIntOrNull() ?: 20
                    val collections = collectionService.getCollections(userId = userId, page = page, pageSize = pageSize)
                    call.respond(HttpStatusCode.OK, collections)
                } catch (e: UnauthorizedException) { throw e }
                catch (e: Exception) { throw ApiException(HttpStatusCode.InternalServerError, "Failed to get collections: ${e.message}", e) }
            }

            get("{id}") {
                val collectionService = call.application.get<CollectionService>()
                try {
                    val id = call.parameters["id"] ?: throw ValidationException("Collection ID is required")
                    val userId = call.getCurrentUserId()
                ...
                catch (e: Exception) { throw ApiException(HttpStatusCode.InternalServerError, "Failed to get collection: ${e.message}", e) }
            }
            ...
            post {
                ...
                catch (e: Exception) { throw ApiException(HttpStatusCode.InternalServerError, "Failed to create collection: ${e.message}", e) }
            }
            ...
            put("{id}") {
                ...
                catch (e: Exception) { throw ApiException(HttpStatusCode.InternalServerError, "Failed to update collection: ${e.message}", e) }
            }
            ...
            delete("{id}") {
                ...
                catch (e: Exception) { throw ApiException(HttpStatusCode.InternalServerError, "Failed to delete collection: ${e.message}", e) }
            }
        }
    }
}
