...
private suspend fun captureFullContentImage(
    ...
    return withContext(Dispatchers.Default) {
        ...
        if (data.writeToFile(path, true)) "file://$path" else null
    }
}
...
private fun trimBottomBackgroundArea(image: UIImage): UIImage {
    ...
    val croppedCg = CGImageCreateWithImageInRect(
        image = cgImage,
        rect = CGRectMake(0.0, 0.0, width.toDouble(), croppedHeight.toDouble()),
    ) ?: return image
    return UIImage.imageWithCGImage(cgImage = croppedCg, scale = image.scale, orientation = image.imageOrientation)
}
