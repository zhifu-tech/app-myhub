...
import tech.zhifu.app.myhub.feature.ai.ActionOptionSchema
...
class ActionPlanner {
    fun actionsFor(
        ...
        return when (state) {
            CaptureState.INFO_COLLECT -> listOf(
                ActionComponentSchema(
                    type = "upload",
                    field = "media",
                    options = listOf(
                        ActionOptionSchema(label = "上传图片", value = "upload_media"),
                    )
                ),
                ActionComponentSchema(
                    type = "tag_selector",
                ...
                ActionComponentSchema(
                    ...
                    options = listOf(
                        ActionOptionSchema(label = "跳过标签", value = "skip_tags"),
                        ActionOptionSchema(label = "进入复核", value = "review"),
                    )
                )
            )
            CaptureState.CARD_REVIEW,
            CaptureState.MANUAL_EDIT -> listOf(
                ActionComponentSchema(
                    type = "input",
                    field = "title",
                    options = listOf(
                        ActionOptionSchema(label = "输入新标题后发送", value = "input_title_hint"),
                    )
                ),
                ActionComponentSchema(
                    type = "card_actions",
                    options = listOf(
                        ActionOptionSchema(label = "编辑标题", value = "edit_title"),
                        ActionOptionSchema(label = "发布卡片", value = "publish"),
                    )
                )
            )
            CaptureState.COMPLETE -> listOf(
                ActionComponentSchema(
                    type = "quick_reply",
                    options = listOf(
                        ActionOptionSchema(label = "新建捕获", value = "new_capture"),
                    )
                )
            )
            else -> emptyList()
    ...
    private fun buildTagOptions(draft: CaptureDraft?): List<ActionOptionSchema> {
