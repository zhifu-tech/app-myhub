# AI捕获系统的Prompt+Tool调度架构（LLM Agent Architecture）V1.0
...
用于直连模型的 System Prompt（建议）：

```text
你是 AI 捕获助手。
仅返回 JSON，不要 markdown 代码块。
必须包含 intent,title,summary,tags 四个字段。
tags 必须是字符串数组。
信息不足时也要返回可编辑草稿，intent 默认 create_card。
```

补充策略：

```text
优先保证 title/summary 可用，再给 1-5 个候选 tags。
禁止输出解释性前后缀文本。
```

## 17.2 Task Context（状态输入）

```json
{
  "conversation_state": "INFO_COLLECT|CARD_REVIEW|MANUAL_EDIT",
  "missing_fields": ["media", "tags", "title"],
  "draft": {
    "title": "string",
    "summary": "string",
    "tags": ["string"],
...
```text
update_title    -> ToolCommand.UpdateTitle
add_tag         -> ToolCommand.AddTag
attach_media    -> ToolCommand.AttachPickedMedia
publish_card    -> ToolCommand.PublishCard
```

约束：

```text
任何数据变更都经 ToolCommandDispatcher。
状态推进由 ConversationEngine + StateMachine 控制。
```

## 17.4 流式推理显示规范

当 Provider 支持 SSE 且返回 `delta.reasoning` 时：

```text
UI 实时展示 reasoning（降低等待焦虑）
最终内容按 delta.content 聚合后再解析 JSON
```
